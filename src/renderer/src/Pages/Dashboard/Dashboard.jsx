  ...
  return (
    <div className="w-full flex flex-wrap gap-2 gap-y-8 justify-between">
      <Card tailStyle={'p-6 rounded-md w-[100%] sm:w-[48%] lg:w-[23%]'}>
        <div className="bg-gray-700 w-10 h-10 rounded-full flex items-center justify-center mb-2">
      ...
      <Card
        cardHeading={'Top Channel'}
        tailStyle={'p-6 rounded-md w-[100%] lg:w-[68%] lg:mx-auto h-fit'}
      >
      ...
      <Card
        cardHeading={'Chats'}
        tailStyle={'p-6 rounded-md w-[100%] lg:w-[50%] lg:mx-auto xl:w-[30%]'}
      >
                ...
                <div className="flex items-center  gap-6 relative mb-2">
                  <div className="w-14 rounded-full relative">
                    ...
                    <div className="w-4 h-4 rounded-full bg-white absolute right-0 bottom-0 flex items-center justify-center">
                      <span
                        className={`w-3 h-3 rounded-full ${person.active ? 'bg-green-700' : person.recentlyActive ? 'bg-yellow-400' : 'bg-gray-400'}`}
                      ></span>
                    </div>
                  </div>
                  <div className={`${[person.seen ? 'text-gray-400' : 'text-white']}`}>
                    ...
                    <div className="flex text-sm gap-2 items-end">
                      <p className="w-32">
                        {person.msg.length < 25 ? person.msg : person.msg.slice(0, 25) + '...'}
                      </p>
                      <span className="text-gray-400 text-xs">{person.time}</span>
                    </div>
                  </div>
                  {!person.seen && (
                    <span className="absolute top-1/2 right-0 w-6 h-6 rounded-full bg-blue-700 text-sm flex items-center justify-center">
                      {person.noOfMsgs}
                    </span>
                  )}
                </div>
